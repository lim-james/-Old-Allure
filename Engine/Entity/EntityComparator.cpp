...
void EntityComparator::Partition(Quad<Entity*>* _root)
{
	...
	for (int i = 0; i < _root->list.size(); i++)
	{
		...
		if (transform->translation.x <= _root->position.x 
			&& transform->translation.z >= _root->position.z
			&& transform->translation.x >= _root->position.x - _root->size.x / 2
			&& transform->translation.z <= _root->position.z + _root->size.z / 2)
		{

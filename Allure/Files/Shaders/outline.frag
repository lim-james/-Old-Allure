...
//	float t = abs(sin(radians(material.et * 50.f))) * 0.5f + 0.5f;
	float mag = clamp(dot(vs_out.normal, viewDirection), 0.f, 1.f);
	float inv = 1.f - mag;
	vec3 outline = inv * material.outlineTint;
	color = vec4(outline + mag * color.xyz, color.a);
//	color = vec4(vec3(outline), diffuse.a);
//	color += material.outlineTint * abs(sin(radians(material.et * vs_out.fragmentPosition.y)));

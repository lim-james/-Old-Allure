...
bool Vulkan::DrawFrame() {
	...
	if (result == VK_ERROR_OUT_OF_DATE_KHR) {
		...
		Console::Error << "Failed to present swap chains.\n";
		return false;
	}
	...
	if (vkQueueSubmit(graphicsQueue, 1, &submitInfo, inFlightFences[currentFrame]) != VK_SUCCESS) {
		Console::Error << "Failed to submit draw command buffer.\n";
		return false;
	}
	...
	if (result == VK_ERROR_OUT_OF_DATE_KHR || result == VK_SUBOPTIMAL_KHR || context->DidResize()) {
		context->ResetResize();
		RecreateSwapChain();
	} else if (result != VK_SUCCESS) {
		Console::Error << "Failed to present swap chain image.\n";
		return false;
	}
...
void Vulkan::PopulateDebugMessageCreateInfo(VkDebugUtilsMessengerCreateInfoEXT& createInfo) {
	createInfo = {};
	createInfo.sType = VK_STRUCTURE_TYPE_DEBUG_UTILS_MESSENGER_CREATE_INFO_EXT;
	createInfo.messageSeverity = VK_DEBUG_UTILS_MESSAGE_SEVERITY_VERBOSE_BIT_EXT | VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT | VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT;
	createInfo.messageType = VK_DEBUG_UTILS_MESSAGE_TYPE_GENERAL_BIT_EXT | VK_DEBUG_UTILS_MESSAGE_TYPE_VALIDATION_BIT_EXT | VK_DEBUG_UTILS_MESSAGE_TYPE_PERFORMANCE_BIT_EXT;
	createInfo.pfnUserCallback = Vulkan::DebugCallback;
}
...
Vulkan::QueueFamilyIndices Vulkan::FindQueueFamilies(const VkPhysicalDevice& device) {
...
Vulkan::SwapChainSupportDetails Vulkan::QuerySwapChainSupport(const VkPhysicalDevice& device) {
...
bool Vulkan::RecreateSwapChain() {
	while (context->GetSize().x == 0.f || context->GetSize().y == 0.f)	 {
		context->FetchSize();
		glfwWaitEvents();
	}
...
bool Vulkan::CreateGraphicsPipeline() {
	Shader vert, frag;
	if (!vert.Create("Files/Shaders/Compiled/vert.spv", VK_SHADER_STAGE_VERTEX_BIT, device) ||
		!frag.Create("Files/Shaders/Compiled/frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT, device))
		return false;
...
bool Vulkan::CreateFramebuffers() {
	...
	for (size_t i = 0; i < swapChainImageViews.size(); ++i) {
		...
		if (vkCreateFramebuffer(device, &framebufferInfo, nullptr, &swapChainFramebuffers[i]) != VK_SUCCESS) {
			Console::Error << "Failed to create framebffer.\n";
			return false;
		}
	}
...
bool Vulkan::CreateCommandBuffers() {
	...
	if (vkAllocateCommandBuffers(device, &allocInfo, commandBuffers.data()) != VK_SUCCESS) {
		Console::Error << "Failed to allocate command buffers.\n";
		return false;
	}
	...
	for (size_t i = 0; i < commandBuffers.size(); ++i) {
		...
		if (vkBeginCommandBuffer(commandBuffers[i], &beginInfo) != VK_SUCCESS) {
			Console::Error << "Failed to begin recording command buffer.\n";
			return false;
		}
		...
		if (vkEndCommandBuffer(commandBuffers[i]) != VK_SUCCESS) {
			Console::Error << "Failed to record command buffers.\n";
			return false;
		}
	}
...
bool Vulkan::CreateSyncObjects() {
	...
	for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i) {
		if (vkCreateSemaphore(device, &semaphoreInfo, nullptr, &imageAvailableSemaphores[i]) != VK_SUCCESS ||
			...
			Console::Error << "Failed to create synchronization objects for a frame.\n";
			return false;
		}
	}
